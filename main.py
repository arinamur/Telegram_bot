    ...
    for quest, anss in TEST.items():
        ...
        for ans in anss:
            keyboard.add(types.InlineKeyboardButton(ans,
                                                    callback_data=f'ans_{QUESTIONS.index(quest)}_{anss.index(ans)}'))
        msg = await call.message.answer(quest, reply_markup=keyboard)
...
async def edit_msg(message):
    await message.edit_text('Время вышло. Идем дальше.', reply_markup=None)
...
async def test(call: types.CallbackQuery):
    ...
    if call.data and call.data.startswith('ans_'):
        call = call.split('_')
        if RIGHT_ANS[int(call[1])] == call[2]:
            RA += 1
